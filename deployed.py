...
wkd_lambda = (lambda x: 0 if x=='Mon' else 
              ...
              4 if x=='Fri' else
              5 if x=='Sat' else 6)
awkd = wkd_lambda(st.selectbox('Select Your week day of arrival', ['Mon','Tue','Wed','Thurs','Fri','Sat','Sun']))
dwkd = wkd_lambda(st.selectbox('Select Your week day of departure', ['Mon','Tue','Wed','Thurs','Fri','Sat','Sun']))
wkend = st.number_input('Enter How Many Weekend Nights are there in stay',min_value=0)
wk = st.number_input('Enter How many weeknights are there in stay',min_value=0)
totn = wkend + wk
